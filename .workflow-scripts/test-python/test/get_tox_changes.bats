...
@test "get_changed_tox_directories" {
  ...
  export CHANGED_FILES="some/path/file_1 services/some/path/file_2.py services/some/path/file_3 services/some/file_4"
  run get_changed_tox_directories
  echo "$output"
  [ "$status" -eq 0 ]
...
@test "look_for_changed_tox_directories" {
  ...
  [ "${lines[11]}" == "::set-output name=should_test::true" ]
  [ "${lines[12]}" == "" ]
}
